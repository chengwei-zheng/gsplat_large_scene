...
class Runner:
    ...
    def train(self):
        ...
        # Advance schedulers to the correct step when resuming
        if cfg.resume is not None and init_step > 0:
            print(f"Advancing schedulers to step {init_step}...")
        ...
        for step in pbar:
            ...
            if masks is not None:
                ...
                # When using masks, only use L1 loss
                # SSIM with masks causes artifacts due to patch-based computation
                ssimloss = l1loss * 0  # placeholder to keep logging consistent
                loss = l1loss
            else:
...
if __name__ == "__main__":
    ...
    # Override refine_stop_iter to 80% of max_steps (for large scenes)
    strategy = cfg.strategy
    target_stop_iter = int(cfg.max_steps * 0.8)
    if isinstance(strategy, DefaultStrategy):
        strategy.refine_stop_iter = target_stop_iter
        print(f"[Config] DefaultStrategy: refine_stop_iter={target_stop_iter} (80% of {cfg.max_steps})")
    elif isinstance(strategy, MCMCStrategy):
        strategy.refine_stop_iter = target_stop_iter
